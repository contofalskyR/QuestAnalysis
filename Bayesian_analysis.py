...
# Modified version of the plot_delta_vs_mutual_information function with added debugging
...
def plot_delta_vs_mutual_information(df, delta_threshold_col, group_name="All Participants", save_dir="."):
    ...
    # Debug: Check if feature_index or alpha_angle exists with different case
    columns_lower = [col.lower() for col in df.columns]
    if 'feature_index' in columns_lower and 'feature_index' not in df.columns:
        print("Warning: 'feature_index' exists with different case")
        # Find the actual column name
        actual_col = df.columns[columns_lower.index('feature_index')]
        print(f"Actual column name: '{actual_col}'")
        # Rename to standardized format
        df['feature_index'] = df[actual_col]
    
    if 'alpha_angle' in columns_lower and 'alpha_angle' not in df.columns:
        print("Warning: 'alpha_angle' exists with different case")
        # Find the actual column name
        actual_col = df.columns[columns_lower.index('alpha_angle')]
        print(f"Actual column name: '{actual_col}'")
        # Rename to standardized format
        df['alpha_angle'] = df[actual_col]
    
    # Debug: Check data types of columns
    print("\nColumn data types:")
    print(df.dtypes)
    ...
    # Debug: check for feature_index column
    if 'feature_index' in df.columns:
        print("\nFound 'feature_index' column!")
        print("Unique feature_index values:", df['feature_index'].unique())
        
        # Check if feature_index is numeric
        if not pd.api.types.is_numeric_dtype(df['feature_index']):
            print("Warning: feature_index is not numeric.")
            print("Converting feature_index to numeric...")
            # Try to convert to numeric, errors become NaN
            df['feature_index'] = pd.to_numeric(df['feature_index'], errors='coerce')
            print("After conversion, unique values:", df['feature_index'].unique())
        
        # Create a mapping from feature_index to alpha_angle
        feature_to_alpha = {i: angle for i, angle in enumerate(alpha_angles)}
        print("Feature to alpha mapping:", feature_to_alpha)
        
        # Make sure we have integers for grouping
        ...
        print("Group sizes by feature_index:")
        print(group_sizes)
    elif 'alpha_angle' in df.columns:
        print("\nFound 'alpha_angle' column!")
        print("Unique alpha_angle values:", df['alpha_angle'].unique())
        
        # Group by alpha_angle
        grouped = df.groupby('alpha_angle')
        
        # Debug: print group sizes
        group_sizes = grouped.size()
        print("Group sizes by alpha_angle:")
        print(group_sizes)
    else:
        print("\nNo feature_index or alpha_angle column found.")
        print("Creating a placeholder feature_index column based on particiapnt data pattern...")
        
        # Debug: Look at the participant column to see if we can extract feature index
        if 'participant' in df.columns:
            print("Sample participant IDs:", df['participant'].head(10).tolist())
            
            # Check if the CSV is already grouped by feature
            if 'feature' in df.columns:
                print("Found 'feature' column. Using this instead.")
                df['feature_index'] = df['feature'].astype(int)
            else:
                # Assign random alpha angles for demonstration
                print("Warning: Creating random feature indices for demonstration")
                df['feature_index'] = np.random.choice(range(5), size=len(df))
        else:
            print("No 'participant' column found either. Cannot extract feature indices.")
            # For demonstration, assigning random alpha angles
            df['feature_index'] = np.random.choice(range(5), size=len(df))
        
        # Group by the newly created feature_index
        grouped = df.groupby('feature_index')
    
    # Calculate means and errors for each alpha angle/mutual info value
    for i, angle in enumerate(alpha_angles):
        # If grouping by feature_index, use i directly
        if 'feature_index' in df.columns:
            if i in grouped.groups:
                ...
                if len(group) > 0:
                    ...
                    summarized_data.loc[i, 'delta_error'] = std_err
                    summarized_data.loc[i, 'n'] = len(group)
                    print(f"Feature index {i} (α={angle}°): Mean={mean_delta:.4f}, SE={std_err:.4f}, n={len(group)}")
                else:
                    print(f"Warning: No data for feature index {i} (α={angle}°)")
            else:
                print(f"Warning: Group for feature index {i} (α={angle}°) not found")
        
        # If grouping by alpha_angle, use angle directly
        elif 'alpha_angle' in df.columns:
            if angle in grouped.groups:
                group = grouped.get_group(angle)
                ...
                if len(group) > 0:
                    ...
                    summarized_data.loc[i, 'delta_error'] = std_err
                    summarized_data.loc[i, 'n'] = len(group)
                    print(f"Alpha angle {angle}°: Mean={mean_delta:.4f}, SE={std_err:.4f}, n={len(group)}")
                else:
                    print(f"Warning: No data for alpha angle {angle}°")
            else:
                print(f"Warning: Group for alpha angle {angle}° not found")
...
# Updated main function with enhanced error handling 
def main():
