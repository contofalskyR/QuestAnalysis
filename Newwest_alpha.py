...
def process_data_by_learning_status(data_dir, file_pattern='*.csv', ipl=0.95):
    """
    Process data and create separate analyses for learners and non-learners.
    Compare all trials vs last 5 trials methods for each group.
    ...
    # Process each group using both all trials and last 5 trials methods
    results = {}
    
    # Process learners
    print("\n--- Processing Learners Group ---\n")
    
    # Standard processing for comparison purposes
    learner_standard_results = process_group(combined_data, learners, "Learners", 
                                           estimation_method='mean', use_last_five=True, ipl=ipl)
    
    # Enhanced threshold analysis comparing methods
    learner_enhanced_results = analyze_threshold_estimation(combined_data, learners, "Learners", 
                                                          alpha_angles, ipl)
    
    results['learners'] = {
        'standard': learner_standard_results,
        'enhanced': learner_enhanced_results
    }
    
    # Process non-learners
    print("\n--- Processing Non-Learners Group ---\n")
    
    # Standard processing for comparison purposes
    non_learner_standard_results = process_group(combined_data, non_learners, "Non-Learners", 
                                               estimation_method='mean', use_last_five=True, ipl=ipl)
    
    # Enhanced threshold analysis comparing methods
    non_learner_enhanced_results = analyze_threshold_estimation(combined_data, non_learners, "Non-Learners", 
                                                              alpha_angles, ipl)
    
    results['non_learners'] = {
        'standard': non_learner_standard_results,
        'enhanced': non_learner_enhanced_results
    }
    
    # Create comparison between learners and non-learners for both methods
    create_learner_nonlearner_comparison(learner_enhanced_results, non_learner_enhanced_results, alpha_angles)
    
    # Print summary of findings
    print("\n=== Analysis Summary ===\n")
    print(f"1. Learners (All Trials): R² = {learner_enhanced_results['all_trials']['r_squared']:.3f}")
    print(f"2. Learners (Last 5): R² = {learner_enhanced_results['last5_trials']['r_squared']:.3f}")
    print(f"3. Non-Learners (All Trials): R² = {non_learner_enhanced_results['all_trials']['r_squared']:.3f}")
    print(f"4. Non-Learners (Last 5): R² = {non_learner_enhanced_results['last5_trials']['r_squared']:.3f}")
...
def analyze_threshold_estimation(data, participant_list, group_name, alpha_angles, ipl=0.95):
    """
    Analyze threshold estimation reliability by comparing different methods
    and checking convergence patterns. Also calculate mutual information correlation
    for both methods.
    ...
    Returns:
    --------
    dict: Results with both methods' threshold comparisons and mutual information analysis
    """
    # Filter data for this group
    group_data = data[data['participant'].isin(participant_list)]
    ...
    # 4. Plot average staircase trajectories for this group
    ...
    # 5. NEW ADDITION: Analyze mutual information relationship for both methods
    # Create dictionaries similar to alpha_thresholds for mutual information analysis
    alpha_thresholds_all_trials = {}
    alpha_thresholds_last5 = {}
    
    for alpha in alpha_values:
        alpha_data = results_df[results_df['alpha'] == alpha]
        
        alpha_thresholds_all_trials[alpha] = {
            'pre_threshold': alpha_data['pre_all_trials'].tolist(),
            'post_threshold': alpha_data['post_all_trials'].tolist(),
            'delta_threshold': alpha_data['delta_all_trials'].tolist(),
            'participants': alpha_data['participant'].tolist()
        }
        
        alpha_thresholds_last5[alpha] = {
            'pre_threshold': alpha_data['pre_last5'].tolist(),
            'post_threshold': alpha_data['post_last5'].tolist(),
            'delta_threshold': alpha_data['delta_last5'].tolist(),
            'participants': alpha_data['participant'].tolist()
        }
    
    # Calculate mutual information and analyze correlation for all trials
    mutual_info_all, delta_means_all, r_squared_all = plot_delta_vs_mutual_information(
        alpha_thresholds_all_trials, alpha_values, f"{group_name}_All_Trials", ipl, "_all_trials")
    analyze_mutual_information_relationship(mutual_info_all, delta_means_all, f"{group_name}_All_Trials")
    
    # Calculate mutual information and analyze correlation for last 5 trials
    mutual_info_last5, delta_means_last5, r_squared_last5 = plot_delta_vs_mutual_information(
        alpha_thresholds_last5, alpha_values, f"{group_name}_Last5_Trials", ipl, "_last5")
    analyze_mutual_information_relationship(mutual_info_last5, delta_means_last5, f"{group_name}_Last5_Trials")
    
    # 6. NEW ADDITION: Create a direct comparison of mutual information correlation
    plt.figure(figsize=(10, 6))
    
    # Plot both methods on the same graph
    plt.plot(mutual_info_all, delta_means_all, 'o-', color='blue', label='All trials', 
             linewidth=2, markersize=8)
    plt.plot(mutual_info_last5, delta_means_last5, 's-', color='red', label='Last 5 trials', 
             linewidth=2, markersize=8)
    
    # Add annotations for alpha angles
    for i, (mi, delta_all, delta_last5, angle) in enumerate(zip(
            mutual_info_all, delta_means_all, delta_means_last5, alpha_values)):
        plt.annotate(f'α={angle:.1f}°', 
                    xy=(mi, delta_all),
                    xytext=(5, 5),
                    textcoords='offset points')
    ...
    plt.text(0.05, 0.95, 
             f"All trials: R² = {r_squared_all:.3f}\nLast 5 trials: R² = {r_squared_last5:.3f}", 
    ...
    plt.xlabel('Mutual Information (bits)', fontsize=12)
    plt.ylabel('Delta Threshold (Pre - Post)', fontsize=12)
    plt.title(f'Delta Threshold vs Mutual Information - Method Comparison ({group_name})', fontsize=14)
    plt.legend()
    plt.grid(True, alpha=0.3)
    ...
    # Save figure
    plt.savefig(f"delta_vs_mi_method_comparison_{group_name.replace(' ', '_')}.png", dpi=300, bbox_inches='tight')
    
    print(f"Method comparison for mutual information analysis saved to delta_vs_mi_method_comparison_{group_name.replace(' ', '_')}.png")
    
    # Return comprehensive results including mutual information analysis for both methods
    return {
        'results_df': results_df,
        'all_trials': {
            'delta_means': delta_all_means,
            'mutual_info': mutual_info_all,
            'r_squared': r_squared_all
        },
        'last5_trials': {
            'delta_means': delta_last5_means,
            'mutual_info': mutual_info_last5,
            'r_squared': r_squared_last5
        },
        'correlation_between_methods': correlation
    }
    
def create_learner_nonlearner_comparison(learner_analysis, non_learner_analysis, alpha_angles):
    """
    Create comparison plots between learners and non-learners for both methods.
    
    Parameters:
    -----------
    learner_analysis : dict
        Results from analyze_threshold_estimation for learners
    non_learner_analysis : dict
        Results from analyze_threshold_estimation for non-learners
    alpha_angles : list
        List of alpha angles
    """
    # Comparison for all trials method
    plt.figure(figsize=(10, 6))
    
    # Get data for all trials method
    learner_delta = learner_analysis['all_trials']['delta_means']
    non_learner_delta = non_learner_analysis['all_trials']['delta_means']
    
    # Plot data
    plt.plot(alpha_angles, learner_delta, 'o-', color='blue', 
             label='Learners', linewidth=2, markersize=8)
    plt.plot(alpha_angles, non_learner_delta, 's-', color='red', 
             label='Non-Learners', linewidth=2, markersize=8)
    
    # Add R² values
    plt.text(0.05, 0.95, 
             f"Correlation with MI:\nLearners: R² = {learner_analysis['all_trials']['r_squared']:.3f}\n"
             f"Non-Learners: R² = {non_learner_analysis['all_trials']['r_squared']:.3f}", 
             transform=plt.gca().transAxes,
             bbox=dict(facecolor='white', alpha=0.7),
             verticalalignment='top')
    ...
    plt.xlabel('Alpha Angle (degrees)', fontsize=12)
    plt.ylabel('Delta Threshold (Pre - Post)', fontsize=12)
    plt.title('Learners vs Non-Learners - All Trials Method', fontsize=14)
    plt.legend()
    plt.grid(alpha=0.3)
    ...
    # Save figure
    plt.savefig("learners_vs_nonlearners_all_trials.png", dpi=300, bbox_inches='tight')
    
    # Comparison for last 5 trials method
    plt.figure(figsize=(10, 6))
    
    # Get data for last 5 trials method
    learner_delta = learner_analysis['last5_trials']['delta_means']
    non_learner_delta = non_learner_analysis['last5_trials']['delta_means']
    ...
    # Add R² values
    plt.text(0.05, 0.95, 
             f"Correlation with MI:\nLearners: R² = {learner_analysis['last5_trials']['r_squared']:.3f}\n"
             f"Non-Learners: R² = {non_learner_analysis['last5_trials']['r_squared']:.3f}", 
             transform=plt.gca().transAxes,
             bbox=dict(facecolor='white', alpha=0.7),
             verticalalignment='top')
    ...
    plt.xlabel('Alpha Angle (degrees)', fontsize=12)
    plt.ylabel('Delta Threshold (Pre - Post)', fontsize=12)
    plt.title('Learners vs Non-Learners - Last 5 Trials Method', fontsize=14)
    plt.legend()
    plt.grid(alpha=0.3)
    ...
    # Save figure
    plt.savefig("learners_vs_nonlearners_last5.png", dpi=300, bbox_inches='tight')
    
    # Create combined plot for mutual information analysis
    plt.figure(figsize=(12, 8))
    
    # Get mutual information data
    mi_values = learner_analysis['all_trials']['mutual_info']
    
    # Set up subplots
    plt.subplot(2, 1, 1)  # Top plot - All trials
    
    # Plot data for all trials
    plt.plot(mi_values, learner_analysis['all_trials']['delta_means'], 'o-', color='blue', 
             label='Learners', linewidth=2, markersize=8)
    plt.plot(mi_values, non_learner_analysis['all_trials']['delta_means'], 's-', color='red', 
             label='Non-Learners', linewidth=2, markersize=8)
    
    # Add R² values
    plt.text(0.05, 0.95, 
             f"R² values:\nLearners: {learner_analysis['all_trials']['r_squared']:.3f}\n"
             f"Non-Learners: {non_learner_analysis['all_trials']['r_squared']:.3f}", 
             transform=plt.gca().transAxes,
             bbox=dict(facecolor='white', alpha=0.7),
             verticalalignment='top')
    ...
    plt.title('ΔThreshold vs Mutual Information - All Trials Method')
    plt.legend()
    plt.grid(True, alpha=0.3)
    
    # Bottom plot - Last 5 trials
    plt.subplot(2, 1, 2)
    
    # Plot data for last 5 trials
    plt.plot(mi_values, learner_analysis['last5_trials']['delta_means'], 'o-', color='blue', 
             label='Learners', linewidth=2, markersize=8)
    plt.plot(mi_values, non_learner_analysis['last5_trials']['delta_means'], 's-', color='red', 
             label='Non-Learners', linewidth=2, markersize=8)
    
    # Add R² values
    plt.text(0.05, 0.95, 
             f"R² values:\nLearners: {learner_analysis['last5_trials']['r_squared']:.3f}\n"
             f"Non-Learners: {non_learner_analysis['last5_trials']['r_squared']:.3f}", 
             transform=plt.gca().transAxes,
             bbox=dict(facecolor='white', alpha=0.7),
             verticalalignment='top')
    ...
    plt.xlabel('Mutual Information (bits)')
    plt.ylabel('Delta Threshold (Pre - Post)')
    plt.title('ΔThreshold vs Mutual Information - Last 5 Trials Method')
    plt.legend()
    plt.grid(True, alpha=0.3)
    
    # Add annotations for alpha angles
    for i, (mi, angle) in enumerate(zip(mi_values, alpha_angles)):
        plt.subplot(2, 1, 1)
        plt.annotate(f'α={angle:.1f}°', 
                    xy=(mi, (learner_analysis['all_trials']['delta_means'][i] + 
                            non_learner_analysis['all_trials']['delta_means'][i])/2),
                    xytext=(0, 10),
                    textcoords='offset points',
                    ha='center')
        ...
        plt.subplot(2, 1, 2)
        plt.annotate(f'α={angle:.1f}°', 
                    xy=(mi, (learner_analysis['last5_trials']['delta_means'][i] + 
                            non_learner_analysis['last5_trials']['delta_means'][i])/2),
                    xytext=(0, 10),
                    textcoords='offset points',
                    ha='center')
    ...
    # Save figure
    plt.savefig("delta_vs_mi_comparison_all_methods.png", dpi=300, bbox_inches='tight')
    print("Comparison plots saved to learners_vs_nonlearners_all_trials.png, learners_vs_nonlearners_last5.png, and delta_vs_mi_comparison_all_methods.png")
        
# Main function to run the comprehensive analysis
def run_comprehensive_analysis(data_dir, ipl=0.95):
    """
    Run a comprehensive analysis that compares all trials vs last 5 trials methods
    for threshold estimation, both for the whole sample and split by learning status.
    ...
    print(f"\n\n=== Running comprehensive analysis for Experiment 3 (IPL={ipl}) ===\n")
    
    # Step 1: Get the raw data for all participants
    import glob
    ...
    # Step 3: Run the detailed threshold analysis for all participants
    print("\n--- Analyzing all participants with both threshold methods ---\n")
    all_participants = combined_data['participant'].unique().tolist()
    
    all_participants_analysis = analyze_threshold_estimation(
        combined_data, all_participants, "All_Participants", alpha_angles, ipl)
    
    # Step 4: Identify learners and non-learners
    learner_status = identify_learners(combined_data)
    ...
    # Step 5: Run the detailed threshold analysis for learners and non-learners
    print("\n--- Analyzing learners with both threshold methods ---\n")
    learner_analysis = analyze_threshold_estimation(
        combined_data, learners, "Learners", alpha_angles, ipl)
    
    print("\n--- Analyzing non-learners with both threshold methods ---\n")
    non_learner_analysis = analyze_threshold_estimation(
        combined_data, non_learners, "Non-Learners", alpha_angles, ipl)
    
    # Step 6: Create comparison between learners and non-learners
    create_learner_nonlearner_comparison(
        learner_analysis, non_learner_analysis, alpha_angles)
    
    print("\n=== Analysis complete ===\n")
    print("Summary of findings:")
    print(f"1. All Participants (All Trials): R² = {all_participants_analysis['all_trials']['r_squared']:.3f}")
    print(f"2. All Participants (Last 5): R² = {all_participants_analysis['last5_trials']['r_squared']:.3f}")
    print(f"3. Learners (All Trials): R² = {learner_analysis['all_trials']['r_squared']:.3f}")
    print(f"4. Learners (Last 5): R² = {learner_analysis['last5_trials']['r_squared']:.3f}")
    print(f"5. Non-Learners (All Trials): R² = {non_learner_analysis['all_trials']['r_squared']:.3f}")
    print(f"6. Non-Learners (Last 5): R² = {non_learner_analysis['last5_trials']['r_squared']:.3f}")
    
    return {
        'all_participants': all_participants_analysis,
        'learners': learner_analysis,
        'non_learners': non_learner_analysis
    }
...
if __name__ == "__main__":
    ...
    run_comprehensive_analysis(data_dir, ipl=0.95)
