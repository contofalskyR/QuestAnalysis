...
def analyze_cluster_details(df, expert_profiles, cluster_id, participant_col='participant', 
                          feature_col='feature_index', pre_col='pre_threshold_mean', 
                          post_col='post_threshold_mean', save_dir='.'):
    """
    Detailed analysis of a specific cluster's threshold patterns.
    
    Parameters:
    -----------
    df : pandas.DataFrame
        Original data frame with threshold information
    expert_profiles : list
        List of expert profile dictionaries from perceptual_strategy_analysis
    cluster_id : int
        ID of the cluster to analyze in detail
    participant_col, feature_col, pre_col, post_col : str
        Column names in the dataframe
    save_dir : str
        Directory to save output figures
    """
    # Get the participant IDs for the specified cluster
    cluster_participants = [p['participant'] for p in expert_profiles if p.get('cluster') == cluster_id]
    
    if not cluster_participants:
        print(f"No participants found in cluster {cluster_id}")
        return None
    
    print(f"\n=== DETAILED ANALYSIS OF CLUSTER {cluster_id} ===")
    print(f"Participants in cluster: {cluster_participants}")
    
    # Extract data for this cluster
    cluster_data = df[df[participant_col].isin(cluster_participants)]
    
    # Summary of raw thresholds by feature
    threshold_summary = cluster_data.groupby(feature_col)[[pre_col, post_col]].agg(['mean', 'std'])
    print("\nThreshold Summary by Feature Index:")
    print(threshold_summary)
    
    # Calculate average threshold change
    threshold_change = threshold_summary[post_col]['mean'] - threshold_summary[pre_col]['mean']
    print("\nAverage Threshold Change (post - pre):")
    print(threshold_change)
    
    # Individual participant analysis
    plt.figure(figsize=(15, 5 * len(cluster_participants)))
    
    for i, participant in enumerate(cluster_participants):
        participant_data = cluster_data[cluster_data[participant_col] == participant]
        
        # Add subplot for this participant
        plt.subplot(len(cluster_participants), 1, i+1)
        
        # Get data for each feature index
        for feature_idx, angle in enumerate(alpha_angles):
            feature_data = participant_data[participant_data[feature_col] == feature_idx]
            
            if not feature_data.empty:
                pre = feature_data[pre_col].values[0]
                post = feature_data[post_col].values[0]
                
                # Plot pre and post thresholds
                plt.plot([0, 1], [pre, post], 'o-', 
                         label=f'α={angle}°', linewidth=2, markersize=8)
        
        plt.xlabel('Training', fontsize=12)
        plt.ylabel('Threshold', fontsize=12)
        plt.xticks([0, 1], ['Pre', 'Post'])
        plt.title(f'Participant {participant} Thresholds', fontsize=14)
        plt.legend()
        plt.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'cluster{cluster_id}_detailed_analysis.png'), dpi=300)
    plt.close()
    
    # Compare this cluster to others
    if 'is_expert' in df.columns:
        # Get all experts not in this cluster
        other_experts = df[(df['is_expert']) & (~df[participant_col].isin(cluster_participants))]
        
        # Compare pre-test thresholds
        plt.figure(figsize=(10, 6))
        
        # Group by feature index
        this_cluster_means = cluster_data.groupby(feature_col)[pre_col].mean()
        other_cluster_means = other_experts.groupby(feature_col)[pre_col].mean()
        
        plt.plot(range(len(alpha_angles)), this_cluster_means, 'o-', 
                 label=f'Cluster {cluster_id}', linewidth=2, color='red')
        plt.plot(range(len(alpha_angles)), other_cluster_means, 's-', 
                 label='Other Experts', linewidth=2, color='blue')
        
        plt.xticks(range(len(alpha_angles)), [f'{a}°' for a in alpha_angles])
        plt.xlabel('Alpha Angle', fontsize=12)
        plt.ylabel('Pre-Training Threshold', fontsize=12)
        plt.title(f'Pre-Training Thresholds: Cluster {cluster_id} vs. Others', fontsize=14)
        plt.legend()
        plt.grid(True, alpha=0.3)
        
        plt.tight_layout()
        plt.savefig(os.path.join(save_dir, f'cluster{cluster_id}_vs_others.png'), dpi=300)
        plt.close()
    
    return {
        'participants': cluster_participants,
        'threshold_summary': threshold_summary.to_dict(),
        'threshold_change': threshold_change.tolist()
    }
